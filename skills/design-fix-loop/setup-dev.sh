...
#   4. the skill is USER-level: ~/.claude-shared/skills/design-fix-loop → <checkout>/skills/design-fix-loop,
#      linked from ~/.claude/skills and ~/.claude-personal/skills (whichever profiles exist)
...
# 4. user-level skill symlinks (shared → both profiles), same convention as other shared skills
...
for profile in "$HOME/.claude" "$HOME/.claude-personal"; do
  if [ -d "$profile" ]; then
    ...
    ln -sfn "$HOME/.claude-shared/skills/design-fix-loop" "$profile/skills/design-fix-loop"
    say "skill: $profile/skills/design-fix-loop → ~/.claude-shared → $SKILL_SRC"
  fi
done

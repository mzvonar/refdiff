...
describe("matchElements", () => {
  ...
  it("slot pass never pairs non-text or far-apart elements", () => {
    ...
    const disabled = matchElements([el("d1", 0, 0, 50, 14, "a")], [el("i1", 0, 0, 300, 14, "b")], {
      slotMaxGamma: 0,
    })
  ...
  it("keeps the same-text band bounded: a `5` badge never pairs with a `5` chip three rows away", () => {
    ...
    expect(near.matches.map((m) => [m.design.id, m.impl.id, m.via])).toEqual([
      ["badge", "chip", "text"],
    ])
  })
  ...
  describe("the unrelated-text veto", () => {
    ...
    it("keeps copy drift, which shares tokens", () => {
      ...
      expect(
        veto(
          d,
          i,
          [d, el("x", 0, 0, 130, 14, "Doklad · 12. 7. 2026")],
          [i, el("y", 0, 0, 120, 14, "Blok · 12. 7. 2026")],
        ),
      ).toBe(false)
    })
    ...
    it("needs BOTH counterparts — one is not proof", () => {
      ...
      const oneWay = unrelatedPairing(
        d,
        i,
        gamma(d, i),
        texts([d]),
        texts([i, el("m", 0, 0, 49, 14, "missing")]),
      )

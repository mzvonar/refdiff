...
export function classifyPage(requestedUrl: string, s: PageSignals): PageVerdict {
  ...
  if (s.hasPasswordField && finalPath !== requestedPath)
    return { kind: "login", finalUrl: s.finalUrl }
  ...
  if (inHeading)
    return {
      kind: "error-page",
      detail: `heading "${s.heading.slice(0, 80)}" (matched "${inHeading}")`,
    }
  if (s.bodyText.length < 300) {
    const inBody = ERROR_TITLE_RE.exec(s.bodyText)?.[0]
    if (inBody)
      return { kind: "error-page", detail: `near-empty page says "${s.bodyText.slice(0, 120)}"` }
  }
  return { kind: "ok" }
}
...
async function authenticate(
  ...
  try {
    ...
    if (!res.ok())
      return err(`POST ${auth.url} → HTTP ${res.status()} ${(await res.text()).slice(0, 200)}`)
    return ok(undefined)
  } catch (e) {
...
export async function captureLiveUrl(
  ...
  try {
    ...
    const signals: PageSignals = await page.evaluate(() => ({
      ...
      heading:
        (
          document.querySelector("h1, h2, [role=heading]") as HTMLElement | null
        )?.innerText.trim() ?? "",
    ...
    const extraction = await extractElementTree(page, rootSelector, {
      viewportOrigin: !source.selector,
    })
    if (!extraction || extraction.elements.length === 0 || extraction.height < 20) {
      return err({
        kind: "blank-render",
        ref: identity,
        detail: `${source.url} rendered no visible leaf elements`,
      })
    }

...
describe("runPixelChecks", () => {
  ...
  it("is quiet below the minor threshold", () => {
    ...
    expect(runPixelChecks([squareDiff(m, 6)], [])).toEqual({ findings: [], reported: [] })
  })

  it("emits a pixel-region finding with the tightened region box in CSS px", () => {
    // 20×20 = 400 differing of 1600 → 25% → major
    const [f] = runPixelChecks([squareDiff(m, 20)], []).findings
  ...
  it("maps the region into the design box when the boxes differ in size", () => {
    ...
    const [f] = runPixelChecks([squareDiff(shifted, 20)], []).findings
  ...
  it("does not report a pair the structural channel already covered", () => {
    ...
    expect(runPixelChecks([squareDiff(m, 20)], structural)).toEqual({ findings: [], reported: [] })
  })
  ...
  it("still reports a pair that only has a position finding", () => {
    ...
    expect(runPixelChecks([squareDiff(m, 20)], structural).findings).toHaveLength(1)
  })
  ...
  it("skips text elements by default — element data already describes them", () => {
    ...
    expect(runPixelChecks([squareDiff(text, 20)], [])).toEqual({ findings: [], reported: [] })
    expect(runPixelChecks([squareDiff(text, 20)], [], { skipText: false }).findings).toHaveLength(1)
  })
  ...
  it("reports back only the diffs that produced a finding — the mask paints these", () => {
    const text = match(el("d", { text: "Save" }), el("i", { text: "Save" }))
    const quiet = squareDiff(m, 6)
    const loud = squareDiff(m, 20)
    const { findings, reported } = runPixelChecks([squareDiff(text, 20), quiet, loud], [])
    expect(findings).toHaveLength(1)
    expect(reported).toEqual([{ diff: loud, finding: findings[0] }])
  })

  it("carries the diff's connected regions, largest first, as world-space boxes", () => {
    const [f] = runPixelChecks([squareDiff(m, 20)], []).findings
    // One 20×20 native square at (4,4), dpr 2 → one 10×10 CSS region in the box at (100,50).
    expect(f!.regions).toEqual([{ x: 102, y: 52, w: 10, h: 10 }])
  })

  it("caps the regions it emits", () => {
    const [f] = runPixelChecks([squareDiff(m, 20)], [], { maxRegions: 0 }).findings
...
describe("lowConfidenceFinding", () => {
  it("is a single boxless minor finding carrying the measured confidence", () => {
    const f = lowConfidenceFinding({ scale: 1, offsetX: 0, offsetY: 0, confidence: 0.31 }, 0.5)
    expect(f.type).toBe("pixel-region")

...
const svgFrame = (
  scale: number,
  inner: { x: number; y: number; w: number; h: number; fill: string },
): Buffer => {
  const s = DPR / scale
  return Buffer.from(
...
beforeAll(async () => {
  ...
  await sharp(svgFrame(DESIGN_SCALE, { x: 55, y: 68, w: 30, h: 4, fill: "#1d4ed8" }))
    .png()
    .toFile(designOtherPng)
...
describe("diffMatches (effectful edge)", () => {
  ...
  it("dilates the painted marks and colours them by changeKind", async () => {
    const diffs = await diffMatches(pair(designOtherPng), [match])
    const out = join(dir, "mask-dilated.png")
    await writeDiffMask(
      pair(designOtherPng),
      diffs.map((diff) => ({ diff, finding: { actual: { changeKind: "added" } } })),
      out,
      1,
    )
    const { data } = await sharp(out).ensureAlpha().raw().toBuffer({ resolveWithObject: true })
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      painted++
      if (data[i] === 34 && data[i + 1] === 197 && data[i + 2] === 94) green++
    }
    expect(painted).toBeGreaterThan(diffs[0]!.diffPixels)
    expect(green).toBe(painted)
  })
})

...
>
> **DO NEXT: reconcile step 2's CHEAP TEST** — but first verify the frozen clock on the two
> uctoinak2 corpora and re-baseline (handoff item 1; the freeze is verified only on `refdiff`, and
> the risk on uctoinak2 is auth). Plan steps 0–4 and RECONCILE STEP 1 are DONE; the reconcile
...
> one. Baseline:
> [`baseline-matching-2026-09-15.md`](baseline-matching-2026-09-15.md), regenerated by
> `node scripts/baseline-matching.ts`.
...
**Still to verify: the two uctoinak2 corpora.** Only `refdiff` was re-measured. The specific risk is
AUTH — the capture authenticates via `POST /api/test/session` and the pinned clock sits near session
expiry; the symptom would be `auth-failed` on every pair, which is not a product bug. Expect
`+0 / −0` on Storybook and small movement on the route corpus whose findings name a relative date
(`"August 2026 · uzávierka o 9 dní"`); anything else is real.
